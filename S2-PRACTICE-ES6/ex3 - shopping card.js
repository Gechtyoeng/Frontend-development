...
function getCartTotalAmount() {
  ...
  for(let i = 0;i<PRODUCTS.length;i++){
    let found = SHOPPING_CART.find((f)=> PRODUCTS[i].id === f.id);
    if(found){
      result += (PRODUCTS[i].price * found.quantity);
    }
  }
  return result;
}
...
function addProductToCart(productId) {
  // Write your code here
  let found = SHOPPING_CART.find((f)=> f.id === productId);
  if(found){
    found.quantity++;
  }else{
...
function removeProductFromCart(productId) {
  ...
  let remove = SHOPPING_CART.find((r)=> r.id === productId);
  if(remove.quantity > 1){
    remove.quantity--;
  }else{
    SHOPPING_CART.splice(SHOPPING_CART.indexOf(remove),1);
  }
}
